	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (Pattern.matches("image", part.getContentType().split("/")[0])) {
			...
			if (studentDao.addStudent(student)) {
				int id = studentDao.getStudentId(student.getStudentEmail());
				if (addressDao.addAddress(id, address)) {
					IoUtitlity.uploadImage(part, pathString + fileName);
					response.sendRedirect("/crud/");
				} else {
					throw new CustomerException("cannot add address to database!!!");
				}

			} else {
				throw new CustomerException("Cannot add student to database");
			}
			return;
		}
