...
import com.microsoft.hackthon.dao.TokenRepository;
import com.microsoft.hackthon.dto.*;
import com.microsoft.hackthon.dto.error.response.ErrorResponse;
import com.microsoft.hackthon.dto.request.UserLoginRequest;
import com.microsoft.hackthon.dto.response.JsonResponse;
import com.microsoft.hackthon.dto.response.UserResponse;
...
import com.microsoft.hackthon.service.impl.JwtService;
...
public class UserController {
	...
	public ResponseEntity<JsonResponse<String>> registerUser(@Valid @RequestBody UserInfoDto dto,
			BindingResult result) {
		// if contains error then throwing exception
		if (result.hasErrors()) {
			List<String> validationErrors = result.getAllErrors().stream().map((error) -> error.getDefaultMessage())
					.toList();
		...
		return ResponseEntity.ok(new JsonResponse<>("user", HttpStatus.CREATED,
				"use this token to access protected end points"));
	}
	...
	public ResponseEntity<String> userLogOuts(HttpServletRequest request) {
		...
		List<Token> tokenList = tokenRepository.findTokenByUserName(userName).stream()
				.map((t) -> new Token(t.getTokenId(), t.getTokenStr(), true, true, t.getUser())).toList();
	...
	public ResponseEntity<JsonResponse<String>> loginUser(@Valid @RequestBody UserLoginRequest dto,
		...
		if (!encoder.matches(dto.getPassword(), userInfo.getPassword())) {
			ErrorResponse errorResponse = ErrorResponse.builder().message("username or password is not valid")
		...
		return ResponseEntity.ok(new JsonResponse<>(token.getTokenStr(), HttpStatus.CREATED,
				"use this token to access protected end points"));
	}
